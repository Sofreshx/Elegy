...
`plugins/memory` owns the dedicated `elegy-memory` binary for that surface. `hosts/` keeps only a temporary compatibility bridge for legacy memory commands.

Alongside the existing `elegy` CLI surface, this in-repo `elegy-memory` surface is one of the current shipped operator surfaces.
...
1. `plugins/memory/fixtures/skill.elegy-memory.json` is the governed skill definition source of truth.
2. `plugins/memory/fixtures/skill-discovery-index.elegy-memory.json` is the governed discovery projection derived from that definition.
...
The implemented CLI surface in `plugins/memory/src/cli.rs` is:
...
- [Elegy-memory V1 sibling research note](../research/retention-removal.md)
