...
- **Release plugin archive:** `elegy-memory-plugin-<target>.zip` (primary GitHub release and marketplace contract)
- **Local pack default:** `elegy-memory-v<version>.plugin.zip` (ad hoc output when `pack --output` is omitted)
...
elegy-plugin-packaging install --archive elegy-memory-plugin-<target>.zip
