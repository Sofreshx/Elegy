...
- Lifecycle state transitions are now enforced in v1.1 (see `agent-safe-planning-core/spec.md`). The `--override-transition --reason` flag provides an escape hatch for exceptional recovery.
...
**Note:** Status transitions are now enforced at the storage layer. Invalid transitions are rejected at write time with error `INVALID_STATUS_TRANSITION`. The `--override-transition --reason <text>` flag on `*-update-status` commands provides an explicit override path that records the reason and emits a `STATUS-TRANSITION-OVERRIDDEN` validation warning. See `agent-safe-planning-core/spec.md` for the full transition tables and enforcement rules.
```
...
**Note:** 13 structural-integrity findings (dangling references, cross-scope links, missing parents) are now preflight rejections rather than stored validation warnings. See `agent-safe-planning-core/spec.md` for the promoted findings and promotion principle.
...
3. **Lifecycle transition enforcement is now delivered.** Transition tables for all 8
   ...
   recovery. See `agent-safe-planning-core/spec.md` for details.
