...
`plugins/planning/` is the implementation center. `hosts/`
keeps no permanent compatibility bridge for planning commands.
...
1. `plugins/planning/schemas/skill.schema.json` defines the durable
   capability contract shape.
2. `plugins/planning/fixtures/skill.elegy-planning.json` is the
   governed skill definition source of truth.
3. `plugins/planning/fixtures/skill-discovery-index.elegy-planning.json` is the
   governed discovery projection derived from that definition.
4. `distribution/surfaces.json` (name: `elegy-planning`) is the
   release catalog entry for this surface.
5. SKILL.md mirrors are generated via `elegy plugin export` and are not authoritative.
6. `../specs/index.md` is the implementation-facing spec and the
   authority for entity model, lifecycle, and validation rules.
7. This document (`plugins/planning//docs/architecture/v1.md`) is the
...
The implemented CLI surface in `plugins/planning//src/cli.rs` is:
...
- [elegy-planning Spec](../specs/index.md)
- [Skill Core V1](../../../../docs/architecture/skill-core-v1.md)
- [Elegy Configuration V1](../../elegy-configuration/docs/architecture/v1.md)
- [Elegy Memory V1](../../elegy-memory/docs/architecture/v1.md)
- [Agent Integration](../../../../docs/agent-integration.md)
- [Distribution and downstream consumption](../../../../docs/distribution.md)
- [Substrate Governance (historical)](../../../../docs/architecture/substrate-governance.md)
