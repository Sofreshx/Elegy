...
fn analyze_shared_fixture_matches_expected_analysis_golden() {
    ...
    let descriptor =
        load_json::<McpServerDescriptor>(&fixture_dir.join("mcp-server-descriptor.parity.json"));
    let expected =
        load_json::<McpAnalysisResult>(&fixture_dir.join("mcp-analysis-result.parity.json"));
