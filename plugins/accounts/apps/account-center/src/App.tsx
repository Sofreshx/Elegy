...
import { CloudflareTokenDialog, ConfirmDialog, ConnectDialog, DeviceFlowDialog } from './components/Dialogs'
import { SideNav, type View } from './components/SideNav'
import { seedAccounts } from './types'
import { approveRequest, cancelRequest, connectCloudflareToken, disconnectAccount, loadState, startConnection, type AccountState, type AuthorizationSession, type BrokerRequest, type ConnectionStart } from './api'
import './app.css'
...
export function App({ mode = 'standalone', demo = import.meta.env.DEV }: { mode?: 'standalone' | 'embedded'; demo?: boolean }) {
  ...
  const [state, setState] = useState<AccountState>({ accounts: demo ? seedAccounts : [], requests: demo ? [
    { id: 'demo-request', kind: 'access', status: 'awaiting_user', account_id: seedAccounts[0].id, provider: 'cloudflare', client_id: 'codex-local', purpose: 'read-only DNS research', operations: ['dns.list'], duration_minutes: 60 },
    { id: 'demo-creation', kind: 'creation', status: 'waiting_human', provider: 'github', purpose: 'publish a new project', operations: ['free plan'], duration_minutes: 0 },
  ...
  const [cloudflareCreationUrl, setCloudflareCreationUrl] = useState('https://dash.cloudflare.com/profile/api-tokens')
  const initialConnect = new URLSearchParams(window.location.search).has('connect')
  const [dialog, setDialog] = useState<'connect' | 'cloudflare' | 'review' | 'revoke' | null>(initialConnect ? 'connect' : null)
  ...
  const connectProvider = (provider: string) => {
    if (demo) { setDialog(provider === 'cloudflare' ? 'cloudflare' : null); return }
    setError('')
    void startConnection(provider).then(async connection => {
      ...
      else if (connection.mode === 'manual_token') { setCloudflareCreationUrl(connection.creation_url); setDialog('cloudflare') }
      else { await refresh(); setDeviceFlow(connection) }
    }).catch(caught => { setDialog(null); setError(caught.message) })
  }
  const submitCloudflare = async (token: string) => {
    if (!demo) { await connectCloudflareToken(token); await refresh() }
    setDialog(null)
  }
  ...
  return (
    <div className={`app-shell ${mode}`}>
      ...
      <main className="workspace">
        {view === 'accounts' && <>
          ...
          <div className="account-layout">
            <div className="main-column">
              ...
              {activeAuthorization && <AuthorizationAttention session={activeAuthorization} onReview={() => setDeviceFlow(toConnection(activeAuthorization))} onRetry={() => connectProvider(activeAuthorization.provider)} />}
        ...
        {view === 'requests' && <RequestsView requests={state.requests} onContinue={connectProvider} onCancel={cancelBrokerRequest} />}
      ...
      {dialog === 'connect' && <ConnectDialog onClose={() => setDialog(null)} onProvider={connectProvider} />}
      {dialog === 'cloudflare' && <CloudflareTokenDialog creationUrl={cloudflareCreationUrl} onSubmit={submitCloudflare} onClose={() => setDialog(null)} />}
      ...
      {deviceFlow && <DeviceFlowDialog code={deviceFlow.user_code} verificationUri={deviceFlow.verification_uri} expiresAt={activeAuthorization?.expires_at} onClose={() => setDeviceFlow(null)} />}
    </div>
  )
}

function AuthorizationAttention({ session, onReview, onRetry }: { session: AuthorizationSession; onReview: () => void; onRetry: () => void }) {
...
function toConnection(session: AuthorizationSession): Extract<ConnectionStart, { mode: 'device' }> {
  return { mode: 'device', request_id: session.id, user_code: session.user_code, verification_uri: session.verification_uri, expires_in: Math.max(0, Math.floor((new Date(session.expires_at).getTime() - Date.now()) / 1000)), interval: session.interval_seconds }
}
...
function RequestsView({ requests, onContinue, onCancel }: { requests: BrokerRequest[]; onContinue: (provider: string) => void; onCancel: (request: BrokerRequest) => void }) {
...
function providerLabel(provider: string) {
  return ({ github: 'GitHub', cloudflare: 'Cloudflare' } as Record<string, string>)[provider] ?? `${provider.charAt(0).toUpperCase()}${provider.slice(1)}`
}
