...
impl QueryEngine {
    ...
    pub fn symbol(&self, name: &str, lang: Option<&str>) -> Result<String> {
        ...
        if results.is_empty() {
            ...
            Ok(serde_json::to_string_pretty(
                &QueryResult::<Vec<&Entity>>::not_found(msg),
            )?)
        } else {
    ...
    pub fn neighbors(&self, id: &str, direction: &str) -> Result<String> {
        let dir = match direction {
            ...
            other => {
                return Ok(serde_json::to_string_pretty(&QueryResult::<()>::error(
                    format!("Invalid direction '{}'. Use 'in' or 'out'.", other),
                ))?);
            }
        };
        ...
        if results.is_empty() {
            Ok(serde_json::to_string_pretty(&QueryResult::<
                Vec<(crate::ir::Entity, EdgeKind)>,
            >::not_found(
                format!(
    ...
    pub fn impact(&self, path: &str) -> Result<String> {
        ...
        if file_entities.is_empty() {
            return Ok(serde_json::to_string_pretty(
                &QueryResult::<()>::not_found(format!("No entities found in file '{}'", path)),
            )?);
        }
...
mod tests {
    ...
    fn test_symbol_query_json_format() {
        ...
        let store = Store::open(db_path.to_str().expect("path to str")).expect("open store");
    ...
    fn test_symbol_not_found() {
        ...
        let output = engine.symbol("nonexistent", None).expect("query symbol");
        let parsed: serde_json::Value = serde_json::from_str(&output).expect("parse output JSON");
    ...
    fn test_symbol_lang_filter() {
        ...
        store
            .insert_entity(&make_entity("e1", "helper", "elegy-codegraph-ts"))
            .expect("insert entity");
        store
            .insert_entity(&make_entity("e2", "helper", "elegy-codegraph-rust"))
            .expect("insert entity");

        let engine = QueryEngine::new(store);

        // Without filter, should find both
        let output = engine.symbol("helper", None).expect("query symbol");
        let parsed: serde_json::Value = serde_json::from_str(&output).expect("parse output JSON");
        assert_eq!(parsed["data"].as_array().expect("data is array").len(), 2);

        // With ts filter, should find only the TS one
        let output = engine.symbol("helper", Some("ts")).expect("query symbol");
        let parsed: serde_json::Value = serde_json::from_str(&output).expect("parse output JSON");
        assert_eq!(parsed["data"].as_array().expect("data is array").len(), 1);
    ...
    fn test_neighbors_query() {
        ...
        store.insert_entity(&src).expect("insert entity");
        store.insert_entity(&dst).expect("insert entity");

        store
            .insert_edge(&crate::ir::Edge {
                src: "src1".into(),
                dst: "dst1".into(),
        ...
        let engine = QueryEngine::new(store);
        let output = engine.neighbors("src1", "out").expect("query neighbors");
        let parsed: serde_json::Value = serde_json::from_str(&output).expect("parse output JSON");
        assert_eq!(parsed["status"], "ok");
        assert_eq!(parsed["data"].as_array().expect("data is array").len(), 1);
    ...
    fn test_summary_query() {
        ...
        store
            .insert_entity(&make_entity("e1", "add", "elegy-codegraph-ts"))
            .expect("insert entity");
        store
            .insert_entity(&make_entity("e2", "sub", "elegy-codegraph-ts"))
            .expect("insert entity");
    ...
    fn test_impact_query() {
        ...
        let engine = QueryEngine::new(store);
        let output = engine.impact("lib.rs").expect("query impact");
        let parsed: serde_json::Value = serde_json::from_str(&output).expect("parse output JSON");
        assert_eq!(parsed["status"], "ok");
    ...
    fn test_impact_not_found() {
        ...
        let output = engine.impact("nonexistent.ts").expect("query impact");
        let parsed: serde_json::Value = serde_json::from_str(&output).expect("parse output JSON");
        assert_eq!(parsed["status"], "not_found");
    }
}
