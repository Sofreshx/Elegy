...
//! ```
//!
//! ## Deferred commands (see docs/specs/diff-slice.md)
    ...
    match cli.command {
        ...
        Command::Query { graph, sub } => {
            ...
            let output = match sub {
                QueryCommand::Symbol { name, lang } => engine.symbol(&name, lang.as_deref())?,
                QueryCommand::Neighbors { id, direction } => engine.neighbors(&id, &direction)?,
