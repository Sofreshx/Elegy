...
impl Store {
    ...
    pub fn insert_entity(&self, entity: &Entity) -> Result<()> {
        ...
        // 1. Primary entity storage
        {
            ...
            table
                .insert(entity.id.as_str(), json.as_str())
                .map_err(map_err)?;
        }
    ...
    pub fn get_neighbors(
        ...
        let edges: Vec<(EntityId, EdgeKind)> = match edge_table.get(id.as_str()).map_err(map_err)? {
            Some(guard) => serde_json::from_str(guard.value())?,
            None => return Ok(Vec::new()),
        };
...
mod tests {
    ...
    fn test_open_and_insert_entity() {
        ...
        let store = Store::open(path.to_str().expect("path to str")).expect("open");
        ...
        let found = found.expect("found is some");
    ...
    fn test_compact() {
        let path = temp_db_path();
        let mut store = Store::open(path.to_str().expect("path to str")).expect("open");
    ...
    fn test_missing_entity_returns_none() {
        ...
        assert!(result.is_ok());
        assert!(result.expect("get_by_id ok").is_none());
