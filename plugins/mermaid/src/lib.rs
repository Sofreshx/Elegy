...
pub fn project_from_json_value(
    ...
    match detect_document_kind(value)? {
        DocumentKind::CanonicalWorkflowGraph => {
            ...
            build_canonical_workflow_graph_projection(document)
        }
        DocumentKind::CanonicalWorkflow => {
            ...
            build_canonical_workflow_projection(document)
        }
    }
}
...
mod tests {
    ...
    const CANONICAL_WORKFLOW_GRAPH_FIXTURE: &str = r#"{
  "canonicalFormat": "canonical-workflow-graph",
  "canonicalVersion": 1,
  "entryStepId": "step.collect",
  "trigger": {
    "type": "event",
  ...
  "nodes": [
    {
      ...
      "config": {},
      "inputMappings": { "payload": "trigger.payload" },
      "inputResolutions": {
        "payload": { "sourceExpression": "trigger.payload" }
      },
...
"#;
    const CANONICAL_WORKFLOW_FIXTURE: &str = r#"{
  ...
  "connections": [
    {
      "id": "conn.review-to-fulfill",
      "fromStepId": "step.review",
      "toStepId": "step.fulfill",
      "label": "approved"
    }
  ],
  "layout": {
    ...
    "positions": [
      { "stepId": "step.review", "x": 200, "y": 120 },
      { "stepId": "step.fulfill", "x": 600, "y": 120 }
    ]
  }
}
"#;
