...
| `elegy configuration ...` | Deterministic repo and home materialization plus drift verification from governed templates and profiles. |
...
`elegy configuration ...` is the deterministic materialization lane for
agent-facing repo and home assets after the CLI is already available from a
repo checkout or the release installer.

The same behavior is also available as the dedicated `elegy-configuration`
binary and wrapper surface when a consumer wants only the configuration lane.
...
- `repo-skill-mirror-minimal` for deterministic skill mirroring into a selected harness path
- `repo-opencode-minimal` for repo-local OpenCode skill mirroring, `AGENTS.md` patching, minimal MCP config, and hook config
- `repo-codex-minimal` for repo-local Codex-facing skill mirroring
- `codex-home-minimal` for a bounded Codex home `AGENTS.md`, agent file, and managed `config.toml` block

Bindings such as `authority.skills` are template-driven and overrideable with
`--binding KEY=VALUE`; `.github/skills` is a safe default in current repo
templates, not a universal rule for all consumers.

Use the distribution installer to download and verify release assets. Keep
product-specific bootstrap, auth, policy, and runtime wiring in the consuming
repo.
