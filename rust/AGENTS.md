...
- `elegy-memory-mcp` is a transport adapter over `elegy-memory`, not a new memory authority. Keep scope binding, salience, correction, and audit behavior aligned with `elegy-memory`.
...
- If a transport exposes MCP over HTTP or stdio, verify auth boundaries, scope isolation, stdout/stderr protocol separation, and logging redaction in addition to tool behavior.
