...
`rust/features/elegy-memory` owns the dedicated `elegy-memory` binary for that surface. `rust/bin/elegy-cli` keeps only a temporary compatibility bridge for legacy memory commands.
...
The implemented CLI surface in `rust/features/elegy-memory/src/cli.rs` is:
