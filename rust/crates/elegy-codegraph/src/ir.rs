...
mod tests {
    ...
    fn round_trip_contract_fixture() {
        // From CARGO_MANIFEST_DIR (rust/crates/elegy-codegraph/), go up 3 levels to repo root
        let fixture_path = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../../contracts/fixtures/elegy-codegraph.graph.v0.example.json"
        );
        let json = std::fs::read_to_string(fixture_path)
            .expect("Failed to read contract fixture");

        // Deserialize
        let graph: Graph = serde_json::from_str(&json)
            .expect("Failed to deserialize contract fixture");
        ...
        assert_eq!(graph.entities[2].provenance.confidence, Confidence::Inferred);
        ...
        let re_serialized = serde_json::to_string_pretty(&graph)
            .expect("Failed to re-serialize graph");
        let _reparsed: serde_json::Value = serde_json::from_str(&re_serialized)
            .expect("Re-serialized output is not valid JSON");

        // Verify key fields survive round-trip
        let round_trip: Graph = serde_json::from_str(&re_serialized)
            .expect("Round-trip deserialization failed");
    ...
    fn confidence_serde_round_trip() {
        ...
        for (json, expected) in cases {
            let deser: Confidence = serde_json::from_str(json).unwrap();
    ...
    fn side_effect_serde_round_trip() {
        ...
        for (json, expected) in cases {
            let deser: SideEffect = serde_json::from_str(json).unwrap();
            assert_eq!(deser, expected);
            let ser = serde_json::to_string(&expected).unwrap();
            assert_eq!(ser, json);
        }
    ...
    fn entity_kind_serde_round_trip() {
        ...
        for (json, expected) in cases {
            let deser: EntityKind = serde_json::from_str(json).unwrap();
            assert_eq!(deser, expected);
            let ser = serde_json::to_string(&expected).unwrap();
            assert_eq!(ser, json);
        }
    ...
    fn type_hint_with_and_without_name() {
        ...
        let hint: TypeHint = serde_json::from_str(json).unwrap();
