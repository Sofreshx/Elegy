...
use crate::ir::{
    Confidence, Edge, EdgeKind, Entity, EntityId, EntityKind, ExtractorMeta, Graph,
    Provenance, Span,
};
    ...
    match node.kind() {
        ...
        "const_item" => {
            if let Some(name_node) = node.child_by_field_name("name") {
                ...
                let span = make_span(node, &name_node);
                let entity_id =
                    state.add_entity(EntityKind::Constant, name, "source", span);
                if has_visibility(node) {
                    state.add_edge(file_id, &entity_id, EdgeKind::Exports);
                }
            }
        ...
        "mod_item" => {
            if let Some(name_node) = node.child_by_field_name("name") {
                ...
                let mod_id =
                    state.add_entity(EntityKind::Module, name, "source", span);
        ...
        "use_declaration" => {
            ...
            if let Ok(path_text) = node.utf8_text(source.as_bytes()) {
                ...
                let span = make_span(node, node);
                let import_id =
                    state.add_entity(EntityKind::Module, clean_path, "source", span);
...
pub fn extract(repo_path: &str) -> Result<Graph> {
    ...
    parser
        ...
        .map_err(|e| {
            crate::error::Error::Extraction(format!(
                "Failed to set tree-sitter-rust language: {}",
                e
            ))
        })?;
        ...
        .filter_entry(|e| !e.file_name().to_str().map_or(false, |n| n == "target"))
    {
        let entry = match entry {
            ...
            Err(err) => {
                ...
                eprintln!(
                    "warn: skipping unreadable entry in {}: {}",
                    repo_path, err
                );
                continue;
            }
        ...
        let layer = if file_path.starts_with("tests/")
        ...
        let file_id =
            state.add_entity(EntityKind::File, &file_path, layer, file_span);
...
mod tests {
    ...
    fn test_extract_parses_simple_file() {
        ...
        let graph = extract(dir.path().to_str().unwrap()).unwrap();
