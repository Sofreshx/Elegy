...
pub fn augment(graph: &mut Graph, repo_path: &str) -> Result<()> {
    ...
    let scip_index = match scip::types::Index::parse_from_bytes(&scip_bytes) {
        Ok(index) => index,
        Err(e) => {
            graph.extractor.warning =
                Some(format!("Failed to parse SCIP output: {}", e));
            return Ok(());
        }
    };
    ...
    for doc in &scip_index.documents {
        for occ in &doc.occurrences {
            ...
            let source_entity_id = find_entity_at_location(
                &graph.entities,
                &doc.relative_path,
                start_line_1based,
            );
                    ...
                    if let Some(entity) = graph.entities.iter_mut().find(|e| e.id == *src_id) {
                        if entity.provenance.confidence != Confidence::Exact {
                            entity.provenance.extractor =
                                "elegy-codegraph-rust-scip".to_string();
                        }
                    }
                }
            }
...
fn find_entity_at_location(
    ...
    if let Some(line_val) = line {
        ...
        entities.iter().find(|e| e.file == file).map(|e| e.id.clone())
    }
}
...
fn find_entity_by_symbol(
    ...
    entities
        ...
        .or_else(|| {
            ...
            entities.iter().find(|e| e.name == short).map(|e| e.id.clone())
        })
}
...
mod tests {
    ...
    fn test_find_entity_at_location_exact_line() {
        let entities = vec![make_entity(
            "e1", "foo", "mod::foo", "src/lib.rs", Some((5, 1, 10, 2)),
        )];
    ...
    fn test_find_entity_by_symbol_exact_match() {
        let entities = vec![make_entity(
            "e2", "bar", "crate::mod::bar", "src/mod.rs", None,
        )];
    ...
    fn test_find_entity_by_symbol_fallback_short_name() {
        let entities = vec![make_entity(
            "e3", "baz", "something::different::baz", "src/other.rs", None,
        )];
