...
fn build_invocation_context(
    ...
    InvocationContext {
        ...
        requested_at: "2026-05-28T12:10:00Z".to_string(),
...
fn list_command_machine_output_maps_cleanly_to_invocation_contracts() {
    let output = Command::new(env!("CARGO_BIN_EXE_elegy-configuration"))
        ...
        .output()
        .expect("run elegy-configuration list for conformance");
    ...
    assert_eq!(envelope.correlation_id, "corr-config-map-1");
    ...
    assert!(
        data["templateCount"].is_number(),
        "unexpected list data: {data}"
    );
    assert!(
        data["profileCount"].is_number(),
        "unexpected list data: {data}"
    );

    let request = InvocationRequest {
        request_id: "invoke-config-list-1".to_string(),
        capability_id: "elegy.configuration.list".to_string(),
        input: json!({
        ...
        context: build_invocation_context(
            ...
            "exec-config-list-1",
            "integration-test:elegy-configuration-conformance",
            "trace-config-list-1",
            "elegy-configuration-machine-envelope",
        ),
    };
    ...
    let response = InvocationResponse {
        ...
        failure: None,
        completed_at: Some("2026-05-28T12:10:01Z".to_string()),
        trace_ref: request.context.trace_ref.clone(),
        metadata: BTreeMap::from([
            ("surface".to_string(), "elegy-configuration".to_string()),
            (
                "mappedFrom".to_string(),
                "configuration-list-machine-output".to_string(),
            ),
        ]),
    };
    ...
    let event = ExecutionEvent {
        event_id: "exec-event-config-list-1".to_string(),
        request_id: request.request_id.clone(),
        execution_id: request.context.execution_id.clone(),
        sequence: 1,
        timestamp: "2026-05-28T12:10:01Z".to_string(),
        ...
        message: Some("elegy-configuration list completed successfully".to_string()),
        progress: None,
        failure: None,
        metadata: BTreeMap::from([
            ("surface".to_string(), "elegy-configuration".to_string()),
    ...
    assert!(
        response.output.as_ref().expect("completed output")["data"]["templateCount"].is_number()
    );
}

#[test]
fn invalid_binding_machine_output_maps_cleanly_to_failure_contracts() {
    let target_dir = unique_temp_dir("elegy-configuration-invalid-binding");
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_elegy-configuration"))
        ...
        .output()
        .expect("run elegy-configuration invalid binding conformance");
    ...
    assert_eq!(envelope.correlation_id, "corr-config-invalid-1");
    assert!(envelope.non_interactive);
    ...
    let request = InvocationRequest {
        request_id: "invoke-config-invalid-1".to_string(),
        capability_id: "elegy.configuration.apply".to_string(),
        input: json!({
            "command": ["apply"],
        ...
        context: build_invocation_context(
            "corr-config-invalid-1",
            "exec-config-invalid-1",
            "integration-test:elegy-configuration-conformance",
            "trace-config-invalid-1",
            "elegy-configuration-machine-envelope",
        ),
    };
    let request_validation = validate_invocation_request(&request);
    ...
    let response = InvocationResponse {
        ...
        completed_at: Some("2026-05-28T12:12:00Z".to_string()),
        trace_ref: request.context.trace_ref.clone(),
        metadata: BTreeMap::from([
            ("surface".to_string(), "elegy-configuration".to_string()),
            (
                "mappedFrom".to_string(),
                "configuration-invalid-binding-machine-output".to_string(),
            ),
        ]),
    };
    let response_validation = validate_invocation_response(&response);
    ...
    let event = ExecutionEvent {
        event_id: "exec-event-config-invalid-1".to_string(),
        request_id: request.request_id.clone(),
        execution_id: request.context.execution_id.clone(),
        sequence: 1,
        timestamp: "2026-05-28T12:12:00Z".to_string(),
        event_type: ExecutionEventType::Failed,
        status: ExecutionEventStatus::Failed,
        correlation_id: Some(request.context.correlation_id.clone()),
        trace_ref: request.context.trace_ref.clone(),
        capability_id: Some(request.capability_id.clone()),
        message: Some("elegy-configuration apply failed".to_string()),
        progress: None,
        failure: Some(failure),
        metadata: BTreeMap::from([
            ("surface".to_string(), "elegy-configuration".to_string()),
            ("command".to_string(), "apply".to_string()),
        ]),
    };
    let event_validation = validate_execution_event(&event);
    assert!(
