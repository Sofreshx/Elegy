...
- A harness user who wants a new plugin today must hand-author the package JSON, skill v2, and tool requirements. The harness can help narratively but has no tool surface to drive the creation.
...
- A first-class authoring tool lane (for example, `elegy plugin author` or `elegy plugin new --author` and a corresponding `elegy plugin doctor`) that:
  - Scaffolds plugin JSON + skill v2 + instruction skill in one shot.
  - Walks the user (or harness agent) through capability IDs, side-effect class, and tool binary with sensible defaults derived from the referenced skill.
...
- A clear, machine-readable authoring contract so the host does not have to re-derive schema or convention details. The existing schema and model doc already cover this; the gap is an explicit "authoring API" surface rather than ad-hoc human reading.
