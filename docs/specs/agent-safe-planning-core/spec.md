...
- `rust/crates/elegy-planning/src/model.rs:53-184` — Status enums for all entity types. No transition rules enforced; any status can be set from any other.
- `rust/crates/elegy-planning/src/storage.rs:1849-2159` — `update_status()` method. Parses status string into enum and writes to DB. No transition validation exists for Goal, Roadmap, WorkPoint, Plan, Todo, Issue, ReviewPoint, or Insight. Only ProjectRun has explicit gate checks at lines 2586-2755.
- `rust/crates/elegy-planning/src/storage.rs:3082-3169` — `find_runnable_work_points()`. Ranks only by `ordering_index ASC, id ASC`. No priority/urgency/lease-rank awareness.
- `rust/crates/elegy-planning/src/storage.rs:1628-1704` — `session_context()`. Returns only `entitiesTouched`, `insightsRecorded`, `validationSummary`, `tokenEstimate`. No active project runs, work points, plans, pending todos, open blockers, or recommended next action.
- `rust/crates/elegy-planning/src/storage.rs:4310-4347` — Scope enforcement. Mutations currently allow silent `default` scope in `--json --non-interactive` mode.
- `rust/crates/elegy-planning/src/validation.rs:1-737` — 28 validation findings. All advisory-only (warn/error severity, none block writes). No findings for corrective work without targets, blocked downstream work, or session/lease consistency.
- `rust/crates/elegy-planning/src/model.rs:338-355` — WorkPointRecord has `dependency_ids`, `validation_expectations`, `effort_tier`, `file_scopes`, `tags`. No `kind`, `priority`, `repairsWorkPointIds`, `supersedesWorkPointIds`, or `blocksWorkPointIds`.
- `rust/crates/elegy-planning/src/model.rs:503-511` — `SessionContextBundle` definition. No fields for active runs, work points, plans, pending todos, or blockers.
- `rust/crates/elegy-planning/src/session.rs:1-125` — Session file model. Tracks session ID, scope, timestamps. No active work metadata.
- `docs/specs/elegy-planning.md:469-474` — Current spec explicitly defers lifecycle transition enforcement to v1. This spec delivers that deferred feature.
- `docs/specs/elegy-planning.md` (Critical Analysis, item 3) — "No lifecycle transition enforcement…this is the biggest practical risk. Without transition rules, update-status is semantically equivalent to setting a free-text tag."
- `docs/specs/elegy-planning.md` (Critical Analysis, item 2) — "Advisory-first validation is the right call for an LLM-operated tool." This spec promotes 12 structural-integrity findings to preflight rejection. An ADR (`docs/adr/planning-preflight-boundary.md`) should document the promotion principle and agent UX tradeoffs. See `Drift Notes`.
...
- `rust/crates/elegy-planning/src/model.rs` — Add `WorkPointKind` enum, new `WorkPointRecord` fields, new `SessionContextBundle` fields
- `rust/crates/elegy-planning/src/storage.rs` — Schema migration v8, transition enforcement, `find_runnable_work_points()` ranking, session context builder, scope-explicit gate
- `rust/crates/elegy-planning/src/validation.rs` — New validation findings, promotion of existing findings to preflight
- `rust/crates/elegy-planning/src/cli.rs` — New CLI arguments (`--kind`, `--priority`, `--override-transition`, `--repairs-work-point-id`, `--supersedes-work-point-id`, `--blocks-work-point-id`)
- `rust/crates/elegy-planning/src/service.rs` — Service-layer updates for new arguments and rejection flows
- `rust/crates/elegy-planning/src/session.rs` — Extended session file model with `activeProjectRun`, `lastActiveWorkPointId`, `lastCompletedWorkPointId`
- `rust/crates/elegy-planning/tests/` — Integration tests for all acceptance checks
- `docs/specs/elegy-planning.md` — Update to reflect delivered lifecycle enforcement (remove "deferred to v1" note)
