...
# Block all crates.io publishing; keep advisory crate smoke test
...
2. **Keep `publish-crate.yml` as an advisory smoke test** (renamed
   `Crate Verify`). The workflow runs `cargo verify-project`, `cargo
   test`, and `cargo publish --dry-run` from the root workspace. The
   dry-run step is advisory while `publish = false` is set and must not
   be a required merge check while the crates.io lane is dormant.
...
3. Publish a GitHub Release. The `cargo publish --dry-run` step will pass and
   can be made blocking, followed by the actual `cargo publish`.
...
- The `Crate Verify` workflow serves as an advisory smoke test (manifest
  validity + tests) and a dry-run readiness check for future publishing.
