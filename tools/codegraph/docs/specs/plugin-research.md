...
The IR contract at `tools/codegraph/src/ir.rs` locks two enums as closed sets:
...
- `tools/codegraph/src/ir.rs`: governed IR contract schema (Entity, Edge, Provenance, Confidence, SideEffect enums)
- `tools/codegraph/fixtures/elegy-codegraph.graph.v0.example.json`: example valid graph for contract validation
...
The v0 prototype ships as a single Rust binary `elegy-codegraph` in `tools/codegraph/` with two subcommands:
...
The language-specific extractors feed a language-neutral graph IR defined by the governed schema at `tools/codegraph/src/ir.rs`.
...
- `tools/codegraph/src/ir.rs`
- `tools/codegraph/fixtures/elegy-codegraph.graph.v0.example.json`
- `./diff-slice.md`
...
- **Deferred commands:** `./diff-slice.md` stub spec created; deferred command surface documented in `main.rs` module doc comment. ✅
...
The `diff`, `review`, and `validate` commands are deferred to a follow-up spec. See [`./diff-slice.md`](./diff-slice.md) for the design intent, acceptance criteria template, and integration plan. The deferred command surface is noted in the `main.rs` module doc comment; no stub implementations exist.
