...
/// Scan a skill root. Supports both a direct `SKILL.md` package and a
/// `skills/`-style directory with one skill per subdirectory.
fn scan_skills_dir(
    dir: &Path,
    provenance: SkillProvenance,
    ...
    if direct_skill_md.exists() {
        let loaded = load_skill_file(&direct_skill_md, dir.to_path_buf(), provenance.clone())?;
        skills.push(loaded);
    }
...
fn discover_plugin_skills(plugin_dir: &Path) -> Result<Vec<LoadedSkill>, SkillsSurfaceError> {
    ...
    let raw = fs::read_to_string(&manifest_path).map_err(|e| SkillsSurfaceError::Io {
        path: manifest_path.clone(),
        source: e,
    })?;
    let plugin: ElegyPluginV1 =
        serde_json::from_str(&raw).map_err(|e| SkillsSurfaceError::PluginManifestJson {
            path: manifest_path.clone(),
            source: e,
        })?;
    ...
    let Some(skills_rel) = &plugin.skills else {
        return Ok(Vec::new());
    };

    scan_skills_dir(
        &plugin_dir.join(skills_rel),
