...
3. Validate before publishing a change.
   - Run `validate` on every modified fixture and on the
     `tools/skills/fixtures/` directory before opening a PR.
...
- Envelope: `SkillDiscoveryResult` for search, resolve, and
  validate (declared in
   `tools/skills/schemas/skill-discovery-result.schema.json`).
...
- The governed registry format is declared by
   `tools/skills/schemas/skill.schema.json`. Validate every fixture
...
```text
elegy-skills validate --file tools/skills/fixtures/skill.minimal.json --json
```
...
- Governed source: `tools/skills/fixtures/skill.elegy-skills.json`.
- Discovery projection:
  `tools/skills/fixtures/skill-discovery-index.elegy-skills.json`.
