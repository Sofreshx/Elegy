...
pub enum McpRuntimeError {
    ReadResource(ReadResourceError),
    InvalidJson { uri: String, message: String },
    InvalidDescriptor { uri: String, issues: Vec<String> },
}

impl fmt::Display for McpRuntimeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::ReadResource(error) => write!(f, "{error}"),
            Self::InvalidJson { uri, message } => {
                write!(
                    f,
                    "invalid MCP descriptor JSON for resource URI {uri:?}: {message}"
                )
            }
            Self::InvalidDescriptor { uri, issues } => write!(
                f,
                "invalid MCP descriptor for resource URI {uri:?} with {} issue(s)",
                issues.len()
            ),
        }
    }
...
impl RuntimeState {
    ...
    fn load_mcp_server_descriptor(
        &self,
        uri: &str,
    ) -> Result<McpServerDescriptor, McpRuntimeError> {
        let resource = self
            .read_resource(uri)
            .map_err(McpRuntimeError::ReadResource)?;
        let descriptor: McpServerDescriptor =
            serde_json::from_slice(&resource.bytes).map_err(|error| {
                McpRuntimeError::InvalidJson {
                    uri: uri.to_string(),
        ...
        if !validation.is_valid() {
            return Err(McpRuntimeError::InvalidDescriptor {
                uri: uri.to_string(),
...
mod tests {
    ...
    fn mcp_example_runtime_state(content: String) -> RuntimeState {
        ...
        loaded.resources = vec![NormalizedResource::Static(StaticResource {
            id: "Weather MCP".to_string(),
            uri: "elegy://fs-static-minimal/resource/weather-mcp".to_string(),
            title: Some("Weather MCP".to_string()),
            description: Some("Embedded MCP descriptor".to_string()),
            mime_type: Some("application/json".to_string()),
            content,
            descriptor_path: "tests/mcp-static.toml".to_string(),
        })];
        let policy = policy_from_loaded(&loaded);

        compose_runtime_state(&loaded, &policy).expect("MCP example should compose")
    }
...
"#
    ...
    #[test]
    fn runtime_state_consumes_mcp_descriptor_resources() {
        let state = mcp_example_runtime_state(
            json!({
        ...
        let uri = "elegy://fs-static-minimal/resource/weather-mcp";

        let analysis = state
            .analyze_mcp_server(uri)
            .expect("descriptor resource should analyze");
    ...
    #[test]
    fn runtime_state_reports_invalid_mcp_descriptor_json() {
        let state = mcp_example_runtime_state("{ not valid json".to_string());
        let uri = "elegy://fs-static-minimal/resource/weather-mcp";
        ...
        assert!(matches!(
            error,
            McpRuntimeError::InvalidJson { ref uri, .. }
                if uri == "elegy://fs-static-minimal/resource/weather-mcp"
        ));
    }
