...
elseif (Test-Path $repoInstaller) {
    ...
    throw 'Unable to locate scripts/install-distribution.ps1 for the Elegy-configuration wrapper surface.'
}

$invokeArgs = @{
    Repository = $Repository
    CliSurfaces = @('elegy-configuration')
    WrapperSurfaces = @('elegy-configuration')
}
