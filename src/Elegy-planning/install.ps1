...
elseif (Test-Path $repoInstaller) {
    ...
    throw 'Unable to locate scripts/install-distribution.ps1 for the Elegy-planning wrapper surface.'
}

$invokeArgs = @{
    Repository = $Repository
    CliSurfaces = @('elegy-planning')
    WrapperSurfaces = @('elegy-planning')
}
